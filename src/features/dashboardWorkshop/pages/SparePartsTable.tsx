  ...
  if (parts.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-primary/15 bg-primary-light/40 px-6 py-12 text-center">
        <p className="text-sm font-medium text-primary">لا توجد قطع غيار</p>
          ...
          <tbody className="divide-y divide-primary/5">
            {parts.map((part) => (
              <tr key={part.id} className="transition hover:bg-primary-light/30">
                <td className="whitespace-nowrap px-4 py-4 font-mono text-xs text-primary sm:px-6">
                  {part.sku}
                </td>
                <td className="px-4 py-4 font-medium text-gray-800 sm:px-6">{part.name}</td>
