...
import { useGetBrandsAllQuery, useStoreBrandMutation, useUpdateBrandMutation, useDeleteBrandMutation } from "../../auth/authApi";
import type { AuthValidationErrorResponse, Brand } from "../../../types/authTypes";

const EMPTY_BRANDS: never[] = [];
const PAGE_SIZE = 10;

function formatBrandDate(value?: string | null) {
  if (!value) return "—";
...
function CompaniesPage() {
  ...
  const { data, isLoading, isError, isFetching } = useGetBrandsAllQuery();
  const [storeBrand, { isLoading: isStoring }] = useStoreBrandMutation();
  const [updateBrand, { isLoading: isUpdating }] = useUpdateBrandMutation();
  const [deleteBrand, { isLoading: isDeleting }] = useDeleteBrandMutation();

  const brands = data?.data ?? EMPTY_BRANDS;
  const normalizedQuery = searchQuery.trim().toLowerCase();
  const filteredBrands = normalizedQuery ? brands.filter((brand) => brand.name.toLowerCase().includes(normalizedQuery)) : brands;
  const { pageItems, currentPage } = getPageSlice(filteredBrands, page, PAGE_SIZE);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const brand_name = companyName.trim();

    try {
      if (editingBrand) {
        const result = await updateBrand({ id: editingBrand.id, brand_name }).unwrap();
        setSuccessMessage(result.message || "تم تعديل الشركة بنجاح");
      } else {
        const result = await storeBrand({ brand_name }).unwrap();
        setSuccessMessage(result.message || "تمت إضافة الشركة بنجاح");
      }
      closeForm();
    } catch (error) {
      ...
      if (errors?.brand_name?.[0]) {
        setNameError(errors.brand_name[0]);
        return;
      }

      setSubmitError(errorData?.message || (isEditing ? "تعذر تعديل الشركة، حاول مرة أخرى." : "تعذر إضافة الشركة، حاول مرة أخرى."));
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setSuccessMessage(result.message || "تم حذف الشركة بنجاح");
      setDeleteTarget(null);
    } catch (error) {
      const errorData = (error as { data?: AuthValidationErrorResponse })?.data;
      setDeleteError(errorData?.message || "تعذر حذف الشركة، حاول مرة أخرى.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-primary">الشركات</h1>
          <p className="mt-1 text-sm text-gray-500">إضافة وتعديل وحذف الشركات</p>
        </div>
        <div className="flex flex-nowrap items-center gap-3">
          <TableSearch id="companies-search" value={searchQuery} onChange={handleSearchChange} placeholder="ابحث عن شركة..." />
          <button
            type="button"
            onClick={openAddForm}
      ...
      {isFormOpen ? (
        <section className="rounded-xl border border-primary/10 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="text-base font-semibold text-primary">{isEditing ? "تعديل الشركة" : "إضافة شركة"}</h2>
            <button
              type="button"
              onClick={closeForm}
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="company_name" className="mb-2 block text-sm font-medium text-label">
                اسم الشركة
              </label>
              <input
                id="company_name"
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 px-6 py-12 text-center shadow-sm" role="alert">
          <p className="text-sm font-medium text-red-700">تعذر تحميل الشركات</p>
        ...
        <div className="rounded-xl border border-dashed border-primary/15 bg-primary-light/40 px-6 py-12 text-center">
          <p className="text-sm font-medium text-primary">{isFetching ? "جاري تحديث القائمة..." : "لا توجد شركات"}</p>
          <p className="mt-1 text-xs text-gray-500">ابدأ بإضافة أول شركة من الزر أعلاه</p>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-primary/10 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-176 w-full text-sm">
              ...
              <tbody className="divide-y divide-primary/5">
                {pageItems.map((brand) => (
                  <tr key={brand.id} className="transition hover:bg-primary-light/30">
                    <td className="whitespace-nowrap px-4 py-4 text-center font-medium text-gray-800 sm:px-6">
                      <span className="inline-flex items-center justify-center gap-2">
                        <Building2 className="size-4 text-primary/40" aria-hidden="true" />
                        {brand.name}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4 text-center text-gray-600 sm:px-6">{formatBrandDate(brand.created_at)}</td>
                    <td className="whitespace-nowrap px-4 py-4 text-center text-gray-600 sm:px-6">{formatBrandDate(brand.updated_at)}</td>
                    <td className="whitespace-nowrap px-4 py-4 text-center sm:px-6">
                      <div className="flex flex-nowrap items-center justify-center gap-2">
                        <button
                          type="button"
                          aria-label={`تعديل ${brand.name}`}
          ...
          <Pagination page={currentPage} totalItems={filteredBrands.length} pageSize={PAGE_SIZE} itemLabel="شركة" onPageChange={setPage} />
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        title="حذف الشركة"
        message={deleteTarget ? `هل تريد حذف شركة ${deleteTarget.name}؟` : ""}
        confirmLabel="حذف"
        variant="danger"
